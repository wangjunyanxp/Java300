...
/**
 * 自己实现链表
 * 这里是单向链表，unidirection
 *
 * LinkedList 之 自己实现, 单向链表版
 *   1. size, add, get, indexOf, remove, iterator
 *   2. 支持泛型
 *   3. 支持了迭代器 Iterator
 *      a. public class MyLinkedListUD<T> implements Iterable<T>
 ...
 *
 */
public class MyLinkedListUD<T> implements Iterable<T> {
    Node first;
    Node last;
    int size;

    public MyLinkedListUD() {
        this.first = null;
        this.last = null;
        this.size = 0;
    }
    ...
    private class Iter implements Iterator<T> {
        ...
        public void remove() {
            ...
            MyLinkedListUD.this.remove(lastRet);
            lastRet = -1;
        }
    }
